...
def get_qa_prompt(
    ...
    formatted_documents = []
    formatted_documents_no_gold = []
    for document_index, document in enumerate(documents):
        formatted_documents.append(f"Document [{document_index+1}](Title: {document.title}) {document.text}")
        if not document.hasanswer:
            formatted_documents_no_gold.append(f"Document [{document_index + 1}](Title: {document.title}) {document.text}")
    prompt = prompt_template.format(question=question, search_results="\n".join(formatted_documents))
    negative_prompt = prompt_template.format(question=question, search_results="\n".join(formatted_documents_no_gold))
    return prompt, negative_prompt


def get_qa_prompt_and_gold_prompt(
    question: str, documents: List[Document], mention_random_ordering: bool, query_aware_contextualization: bool
):
    if not question:
        raise ValueError(f"Provided `question` must be truthy, got: {question}")
    ...
    # Format the documents into strings
    formatted_documents = []
    formatted_documents_gold = []
    for document_index, document in enumerate(documents):
        formatted_documents.append(f"Document [{document_index+1}](Title: {document.title}) {document.text}")
        if document.hasanswer:
            formatted_documents_gold.append(f"Document [{document_index + 1}](Title: {document.title}) {document.text}")
    prompt = prompt_template.format(question=question, search_results="\n".join(formatted_documents))
    gold_prompt = prompt_template.format(question=question, search_results="\n".join(formatted_documents_gold))
    return prompt, gold_prompt
